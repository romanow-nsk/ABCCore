...
public interface RestAPIBase {
    ...
    Call<Artifact> createArtifactFromString(@Header("SessionToken") String token,@Query("fileName") String fileName, @Query("name") String name, @Query("base64") boolean base64, @Body JString text);
    //----------------- Для тестирования ----------------------------------------------------------------------
    @Streaming
    @GET ("/api/file/load2")
